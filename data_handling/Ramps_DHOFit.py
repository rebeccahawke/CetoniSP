...
p2 = os.path.join(path, 'Step_-0.75_25_1607473015.912431_all.xlsx')
...
start = 20.125  # start time for ramp
end = start + .75/25*60
timestamps1 = data1['Timestamp']
t_off = 0.25

# data2 = pd.read_excel(p2, 'RFC data')
# start = 20.09  # start time for ramp
...
### make plots
fig, (ax0, ax1, ax2) = plt.subplots(3, 1, figsize=[w, h*2], sharex=True)
...
ax1.plot(time1[1:start_int] - start, linear(time1[1:start_int], *linParamsFall1),
         color='k', linestyle='--')
#Linear fit with gradient = " + str(round(linParamsFall1[0], 4)) + "\n(u = " + str(
...
ax2.plot(time1[1:start_int] - start, linResFall1,
         color='k', label="Residual of fit to model")
ax2.plot(time1[start_int:end_int] - start, hoResRamp1,
         color='k')
